...
hostapd_bss_tr_send(struct hostapd_data *hapd, u8 *addr, bool disassoc_imminent, bool abridged,
		    int disassoc_timer, int validity_period, struct blob_attr *neighbors)
{
	struct blob_attr *cur;
	struct sta_info *sta;
	int nr_len = 0;
			       ...
			       struct blob_attr *msg)
{
	...
	if (tb[BSS_TR_DA_TIMER])
		da_timer = blobmsg_get_u32(tb[BSS_TR_DA_TIMER]) / hapd->iconf->beacon_int;

	if (tb[BSS_TR_VALID_PERIOD])
		valid_period = blobmsg_get_u32(tb[BSS_TR_VALID_PERIOD]) / hapd->iconf->beacon_int;
...
void hostapd_ubus_notify_bss_transition_response(
	...
	const u8 *condidate_list, u16 candidate_list_len)
{
	char *cl_str;
	u16 i;
	...
	if (candidate_list_len > 0) {
		...
		for (i = 0; i < candidate_list_len; i++)
			snprintf(&cl_str[i*2], 3, "%02X", condidate_list[i]);
		blobmsg_add_string_buffer(&b);
	}
